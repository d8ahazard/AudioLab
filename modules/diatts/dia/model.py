...
from modules.diatts.dia.audio import audio_to_codebook, codebook_to_audio
from modules.diatts.dia.config import DiaConfig
from modules.diatts.dia.layers import DiaModel, KVCache
...
class Dia:
    ...
    @classmethod
    def from_pretrained(
        cls, model_name_or_path: str = "nari-labs/Dia-1.6B", device: torch.device | None = None
    ) -> "Dia":
        ...
        Args:
            model_name_or_path: The Hugging Face Hub repository ID (e.g., "NariLabs/Dia-1.6B") or local path to the model.
            device: The device to load the model onto. If None, will automatically select the best available device.
        ...
        if os.path.isdir(model_name_or_path):
            ...
            checkpoint_path = os.path.join(model_name_or_path, "dia-v0_1.pth")
            if not os.path.exists(config_path) or not os.path.exists(checkpoint_path):
                raise FileNotFoundError(f"Config or checkpoint file not found at {model_name_or_path}")
        else:
            config_path = hf_hub_download(repo_id=model_name_or_path, filename="config.json")
            checkpoint_path = hf_hub_download(repo_id=model_name_or_path, filename="dia-v0_1.pth")
    ...
    def generate(
        ...
        if audio_prompt_path is not None:
            ...
            # Ensure audio_prompt is mono
            if audio_prompt.shape[0] > 1:
                audio_prompt = audio_prompt.mean(dim=0).unsqueeze(0)
