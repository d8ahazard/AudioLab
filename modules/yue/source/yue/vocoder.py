...
from modules.yue.source.yue.vocos import VocosDecoder
...
def process_audio(input_compressed, output_file, rescale, device, decoder, soundstream):
    compressed = input_compressed.astype(np.int16)
    ...
    for instrumental_file, vocal_file, base_name in tqdm(pairs):
        ...
        try:
            ...
            instrumental_output = process_audio(
                np.load(instrumental_file, allow_pickle=True).astype(np.int16),
            ...
            vocal_output = process_audio(
                np.load(vocal_file, allow_pickle=True).astype(np.int16),
