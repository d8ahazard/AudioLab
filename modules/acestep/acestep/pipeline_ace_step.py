...
from modules.acestep.acestep.schedulers.scheduling_flow_match_euler_discrete import (
    FlowMatchEulerDiscreteScheduler,
)
from modules.acestep.acestep.schedulers.scheduling_flow_match_heun_discrete import (
...
from modules.acestep.acestep.language_segmentation import LangSegment
from modules.acestep.acestep.music_dcae.music_dcae_pipeline import MusicDCAE
from modules.acestep.acestep.models.ace_step_transformer import ACEStepTransformer2DModel
from modules.acestep.acestep.models.lyrics_utils.lyric_tokenizer import VoiceBpeTokenizer
from modules.acestep.acestep.apg_guidance import (
    apg_forward,
    MomentumBuffer,
    cfg_forward,
    cfg_zero_star,
...
class ACEStepPipeline:
    ...
    def text2music_diffusion_process(
        ...
        elif scheduler_type == "flow_match_euler":
            # For flow_match_euler, use the same scheduler as euler
            scheduler = FlowMatchEulerDiscreteScheduler(
                num_train_timesteps=1000,
                shift=3.0,
            )
        else:
            # Default to euler if unrecognized
            logger.warning(f"Unrecognized scheduler_type: {scheduler_type}, defaulting to 'euler'")
            scheduler = FlowMatchEulerDiscreteScheduler(
                num_train_timesteps=1000,
                shift=3.0,
            )
